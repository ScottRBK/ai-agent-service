...
class TestKnowledgeBaseReranking:
    ...
    def mock_search_results(self):
        ...
        result1.score = 0.8
        result1.document_metadata = {}
        
        result2 = Mock()
        result2.chunk = chunk2
        result2.score = 0.6
        result2.document_metadata = {}
        
        result3 = Mock()
        result3.chunk = chunk3
        result3.score = 0.7
        result3.document_metadata = {}
    ...
    async def test_end_to_end_reranking_with_ollama(self, mock_agent_config_with_reranking, mock_search_results):
        ...
        with patch('app.core.agents.agent_tool_manager.AgentToolManager') as mock_tool_manager:
            mock_tool_manager.return_value.config = mock_agent_config_with_reranking
            ...
            with patch('app.core.providers.manager.ProviderManager') as mock_provider_manager:
                ...
                with patch('app.core.resources.vector_providers.pgvector.PGVectorProvider') as mock_pg_provider:
                    ...
                    with patch.object(agent.knowledge_base, '_rerank_results') as mock_rerank_method:
                        # Mock the rerank method to return reordered results
                        reranked_results = [
                            SearchResult(chunk=mock_search_results[0].chunk, score=0.95, document_metadata={}),
                            SearchResult(chunk=mock_search_results[2].chunk, score=0.85, document_metadata={}),
                            SearchResult(chunk=mock_search_results[1].chunk, score=0.1, document_metadata={})
                        ]
                        mock_rerank_method.return_value = reranked_results
    ...
    async def test_provider_instance_reuse_main_provider(self):
        ...
        with patch('app.core.agents.agent_tool_manager.AgentToolManager') as mock_tool_manager:
            ...
            with patch('app.core.providers.manager.ProviderManager') as mock_provider_manager:
                ...
                with patch('app.core.resources.vector_providers.pgvector.PGVectorProvider'):
            ...
            with patch('app.core.providers.manager.ProviderManager') as mock_provider_manager:
                ...
                with patch('app.core.resources.vector_providers.pgvector.PGVectorProvider') as mock_pg_provider:
                    ...
                    with patch('app.utils.logging.logger') as mock_logger:
                        ...
                        # Should log warning about re-ranking failure
        ...
        with patch('app.core.agents.agent_tool_manager.AgentToolManager') as mock_tool_manager:
            ...
            with patch('app.core.providers.manager.ProviderManager') as mock_provider_manager:
                ...
                with patch('app.core.resources.vector_providers.pgvector.PGVectorProvider') as mock_pg_provider:
                    ...
                    # Vector search should have been called with rerank_limit (50)
                    mock_vector_provider.search_similar.assert_called()
                    search_call_args = mock_vector_provider.search_similar.call_args
                    # Check if limit parameter was passed correctly
                    if 'limit' in search_call_args[1]:
                        assert search_call_args[1]['limit'] == 50  # rerank_limit